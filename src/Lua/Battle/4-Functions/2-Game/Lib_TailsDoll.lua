...
B.SparringPartnerControl = function()
	if not(B.BattleGametype()) or gametyperules & GTR_CAMPAIGN or titlemapinaction then return end
	//Get player counts
	local playercount = #B.Arena.Fighters
	local redcount = #B.Arena.RedFighters
	local bluecount = #B.Arena.BlueFighters
